...
// POST /api/customer/payments/bookings/:bookingId/qr
...
export const generateQrPayment = async (bookingId, userId) => {
  ...
  const transferContent = removeVietnameseTones(`Thanh toan don hang ${booking.code}`);
...
// POST /api/customer/payments/bookings/:bookingId/confirm-demo — nút
// "Demo: đã chuyển khoản" cho THANH TOÁN VÉ qua QR ngân hàng (song song với
// wallet.service.js#confirmTopup, cùng triết lý: không có cổng thanh toán
// thật đứng sau nên tin theo yêu cầu client, đúng như bạn yêu cầu).
export const confirmBankPaymentDemo = async (bookingId, userId) => {
  const booking = await Booking.findOne({ _id: bookingId, user: userId });
  if (!booking) throw makeError("Không tìm thấy đơn đặt vé", 404);

  // Idempotent: gọi lại cho 1 booking đã "confirmed" (vd double-click nút demo,
  // hoặc mất mạng giữa chừng rồi bấm lại) trả về luôn booking hiện tại thay vì
  // báo lỗi — vé (Ticket) cũng không bị tạo trùng nhờ createTicketsForBooking
  // đã tự kiểm tra trước khi tạo.
  if (booking.status === "confirmed") return booking;

  if (booking.status !== "pending") throw makeError("Đơn đặt vé không ở trạng thái chờ thanh toán", 400);
...
export default { generateQrPayment, confirmBankPaymentDemo };
